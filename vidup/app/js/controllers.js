...
function ($scope, $rootScope, $routeParams, $location, cloudinary) {
        ...
        $scope.updateTitle = function () {
            ...
            uploadParams["context"] = "video=" + $scope.title;
            $scope.widget.fileupload('option', 'formData', uploadParams);
        };

        $scope.nowPlaying = "Big Bunny";

        $scope.playVideo = function (videoId, videoTitle) {
            $scope.nowPlaying = videoTitle;
            $('#videoPlayer').html($($.cloudinary.video(videoId + ".mp4", {
                    crop: 'fill'
                }))
        ...
        $scope.widget = $(".cloudinary_fileupload")
            ...
            .on("cloudinarydone", function (e, data) {
                ...
                $("#title").val("");
                $("#fileNamePreview").val("");
                $("#fileUploadProgress").addClass("progress-bar-success").text("Upload Success");
                $scope.$apply();

            }).on("cloudinaryfail", function (e, data) {
                $("#fileUploadProgress").addClass("progress-bar-danger").text("Upload failed");
