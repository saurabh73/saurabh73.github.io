...
(function ($) {
    ...
    TimelineMe.prototype = {
        ...
        _refreshItemPosition: function (item) {
            ...
            if (item && (item.element) && $(document).width() <= 480) {
                item.element
        ...
        _buildItemContent: function (item) {
            ...
            if (item.type == 'smallItem' || item.type == 'bigItem') {
                ...
                var labelContentElem = '<span class="visibile-xs">Hello</span>';
        ...
        if (elmHeight && (!previousHeight || previousHeight != elmHeight)) {
            ...
            setTimeout(function () {
                resolveElementHeightChange(element, {
                    previousHeight: elmHeight,
                    refreshDelay: refreshDelay,
                    level: (level + 1)
                }).then(function (newHeightVal, previousHeightVal, levelVal) {
    ...
    var heightWatcher = function (element, callback) {
        resolveElementHeightChange(element).then(function (newVal, oldVal) {
            var heightEvent = jQuery.Event('timelineMe.itemHeightChanged', {
                elementHeight: newVal,
        ...
        element.on('timelineMe.itemHeightChanged', function (e) {
            callback({
                element: element,
                newVal: e.elementHeight,
                oldVal: e.previousHeight
            });
            resolveElementHeightChange($(e.target), {
                previousHeight: e.elementHeight
